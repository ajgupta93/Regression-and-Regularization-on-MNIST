...
for j=1:5
    ...
    while(iters<n_iters)
        ...
        grad(2:end) = grad(2:end)+(lambda/ntrain);
        w = w - grad;
        iters = iters+1;
        costJtr = 0;
        costJva = 0;
        ...
        e1(iters) = costJtr/ntrain+(sum(w(2:end))*lambda)/ntrain;
        e2(iters) = costJva/nval+(sum(w(2:end))*lambda)/nval;
        e3(iters) = costJte/ntest+(sum(w(2:end))*lambda)/ntest;
...
figure,
hold on,
plot(1:150,ww(1,1:150));
plot(1:150,ww(2,1:150));
plot(1:150,ww(3,1:150));
plot(1:81,ww(4,1:81));
plot(1:80,ww(5,1:80));
legend('100','10','1','0.1','0.01');
xlabel('No of Iterations');
